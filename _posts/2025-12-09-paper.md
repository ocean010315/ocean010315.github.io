...
**[ Effectiveness on Input-Output Alignment ]**  

<div align="center">
<img src="../assets/img/251209_paper/fig6.png" width="600">
</div>

- 입출력의 정합성에 대한 효율성을 측정하는 파트
- 입력 벡터 $e_t$ 와 새롭게 생성된 은닉층에 대한 벡터 $h_t$, $W_a$ 에 의해 변환한 벡터 $e_{t+1}$ 을 비교
- $h_t$ 의 분포와 $e_t$ 의 분포는 그 거리가 멀지만, $e_{t+1}$ 과 $e_t$ 의 분포의 거리는 가까우며, 유사한 임베딩 영역을 공유함
> $e_t$ 와 $e_{t+1}$ 은 입출력 관계이기 때문에 서로 다른 종류의 내용이 포함되었을 것이라고 생각된다. 이 경우에 두 임베딩 벡터의 분포를 비교하는 것이 과연 동일한 언어 표현을 나타낸다고 할 수 있는가? 임베딩 공간을 어떻게 구성해서 분포를 구했는지를 알아야 할 것 같다.

<div align="center">
<img src="../assets/img/251209_paper/fig7.png" width="250">
</div>

- 또한, align된 벡터 $e_{t+1}$ 을 적용했을 때, LatentMAS의 성능이 2.3-5.3% 향상됨

**[ Optimal Latent Step Depth ]**  

<div align="center">
<img src="../assets/img/251209_paper/fig8.png" width="250">
</div>

- 최적의 LatentMAS를 수행하기 위해서 필요한 step의 수는 몇인지를 파악하는 파트
- 일반적으로 step의 수가 증가할 수록 성능이 향상됨 (40-80 step까지)
- 80 step이 넘어가면 성능이 미세하게 하락하는 형상을 보임

## Comment
- 결론 부분은 그냥 LatentMAS에 대한 요약 뿐
- latent space에서의 표현이 텍스트 기반의 표현보다 더 나은 이유를 탐색해보고 싶어짐
- 몇몇 실험 세팅의 디테일이 부족한 듯 하지만, 내 지식에 대한 부족인 것 같기도 해서 알아보고자 하는 마음이 생김